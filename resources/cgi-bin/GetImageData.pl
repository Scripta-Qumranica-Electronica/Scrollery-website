...
sub getCombs {
	...
	my $sql = $dbh->prepare('select scroll_version.scroll_id as scroll_id, scroll_data.name as name, scroll_version.version as version from scroll_version inner join scroll_data on scroll_data.scroll_id = scroll_version.scroll_id where scroll_version.user_id = ? order by scroll_data.name, scroll_version.version') or die
...
sub getManuscriptData {
	my $dbh = shift;
	my $cgi = shift;
	my $sql = $dbh->prepare('SELECT * FROM fragment') or die
			"Couldn't prepare statement: " . $dbh->errstr;
	$sql->execute();
	readResults($sql);
	return;
...
sub getFragmentData {
	...
	my $sql = $dbh->prepare('CALL getMasterImageListings()') or die
			"Couldn't prepare statement: " . $dbh->errstr;
	$sql->execute();
	readResults($sql);
	return;
...
sub getFragmentPos {
	...
	my $cgi = shift;
	my $fragID = $cgi->param('fragID');
	my $sql = $dbh->prepare('SELECT x_pos, y_pos, rotation FROM fragment WHERE id = ?') or die
			"Couldn't prepare statement: " . $dbh->errstr;
	$sql->execute($fragID);
	readResults($sql);
	return;
...
sub getFragmentPicture {
	...
	my $sql = $dbh->prepare('SELECT image.filename as filename, image.is_master as is_master FROM image WHERE side = ?
...
sub getAllFragments {
	my $dbh = shift;
	my $cgi = shift;
	my $sql = $dbh->prepare('CALL getAllFragments()') 
		or die "Couldn't prepare statement: " . $dbh->errstr;
	$sql->execute();
	readResults($sql);
	return;
...
sub getCanonicalCompositions {
	my $dbh = shift;
	my $cgi = shift;
	my $sql = $dbh->prepare('SELECT DISTINCT composition FROM edition_catalog ORDER BY BIN(composition) ASC, composition ASC') 
		or die "Couldn't prepare statement: " . $dbh->errstr;
	$sql->execute();
	readResults($sql);
	return;
...
sub getCanonicalID1 {
	...
	my $sql = $dbh->prepare('SELECT DISTINCT composition, edition_location_1 FROM edition_catalog WHERE composition = ? ORDER BY BIN(edition_location_1) ASC, edition_location_1 ASC') 
		or die "Couldn't prepare statement: " . $dbh->errstr;
...
sub getCanonicalID2 {
	...
	my $sql = $dbh->prepare('SELECT edition_location_2, edition_catalog_id FROM edition_catalog WHERE composition = ? AND edition_location_1 = ? AND edition_side = 0 ORDER BY CAST(edition_location_2 as unsigned)') 
		or die "Couldn't prepare statement: " . $dbh->errstr;
...
sub getInstitutions {
	my $dbh = shift;
	my $cgi = shift;
	my $sql = $dbh->prepare('SELECT DISTINCT institution FROM image_catalog') 
		or die "Couldn't prepare statement: " . $dbh->errstr;
	$sql->execute();
	readResults($sql);
	return;
...
sub getInstitutionPlates {
	...
	my $sql = $dbh->prepare('SELECT DISTINCT institution, catalog_number_1 FROM image_catalog WHERE institution = ? ORDER BY CAST(catalog_number_1 as unsigned)') 
		or die "Couldn't prepare statement: " . $dbh->errstr;
...
sub getInstitutionFragments {
	...
	my $sql = $dbh->prepare('SELECT catalog_number_2, image_catalog_id FROM image_catalog WHERE institution = ? AND catalog_number_1 = ? AND catalog_side = 0 ORDER BY CAST(catalog_number_2 as unsigned)') 
		or die "Couldn't prepare statement: " . $dbh->errstr;
...
sub getPolygon {
	...
	my $sql = $dbh->prepare('select ST_AsText(region_in_master_image) from artefact where master_image_id=?') 
		or die "Couldn't prepare statement: " . $dbh->errstr;
...
sub setMask {
	...
	my $sql = $dbh->prepare('UPDATE image SET mask = ?, mask_path = ?, center_x = ?, center_y = ? WHERE id = ?') or die
			"Couldn't prepare statement: " . $dbh->errstr;
...
sub setFragmentLocation {
	...
	my $x = $cgi->param('x');
	my $y = $cgi->param('y');
	my $sql = $dbh->prepare('UPDATE fragment SET x_pos = ?, y_pos = ? WHERE id = ?') or die
			"Couldn't prepare statement: " . $dbh->errstr;
...
sub setFragmentRotation {
	...
	my $sql = $dbh->prepare('UPDATE fragment SET rotation = ? WHERE id = ?') or die
			"Couldn't prepare statement: " . $dbh->errstr;
