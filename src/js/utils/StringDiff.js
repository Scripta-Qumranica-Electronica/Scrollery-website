...
;(function() {
  ...
  function cursor_normalize_diff(diffs, cursor_pos) {
    ...
    for (let current_pos = 0, i = 0; i < diffs.length; i++) {
      ...
      if (d[0] === DIFF_DELETE || d[0] === DIFF_EQUAL) {
        const next_pos = current_pos + d[1].length
        if (cursor_pos === next_pos) {
          ...
          const split_pos = cursor_pos - current_pos
          const d_left = [d[0], d[1].slice(0, split_pos)]
          const d_right = [d[0], d[1].slice(split_pos)]
  ...
  function fix_cursor(diffs, cursor_pos) {
    const norm = cursor_normalize_diff(diffs, cursor_pos)
    const ndiffs = norm[1]
    const cursor_pointer = norm[0]
    const d = ndiffs[cursor_pointer]
    const d_next = ndiffs[cursor_pointer + 1]
    ...
    if (d == null) {
      ...
      if (d_next != null && d[1] + d_next[1] === d_next[1] + d[1]) {
        ...
        const suffix = d_next[1].slice(d[1].length)
        if (suffix.length > 0) {
  ...
  function merge_tuples(diffs, start, length) {
    ...
    for (let i = start + length - 1; i >= 0 && i >= start - 1; i--) {
      if (i + 1 < diffs.length) {
        const left_d = diffs[i]
        const right_d = diffs[i + 1]
