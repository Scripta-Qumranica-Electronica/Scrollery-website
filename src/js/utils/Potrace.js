...
/* I should probably either optimize this code for 
 * our use case.  Or perhaps figure out how to
 * use https://www.npmjs.com/package/potrace-js.
 * The code is now a bit more optimized.  Many 
 * functions have been commented out, which do
 * not seem necessary for our implementation.
 * If error crop up, they should be reinstated.
 * (This caveat does __not__ apply to the mask 
 * dilation issue that we currently experience.)
 */
 ...
 * 
 * Edited by Bronson Brown-deVost 12.3.2018
 * altered to hardcode a few project specific needs:
 * we can pass a canvas directly to the constructor,
 * the multiplyFactor is the amount which the source
 * image has been shrunk in the viewer (right now
 * the images are shrunk by a factor of 2 [i.e., 50%]);
 * alphamax is set to 0 in order to create a polygon
 * without any bezier curves;
 * created getPolyPath() function for return svg 
...
function Potrace(canvas, multiplyFactor) {
    ...
    function loadBm() {
      ...
      for (i = 0, j = 0; i < l; i += 4, j++) {
        color = imgdataobj.data[i] + imgdataobj.data[i + 1] +
            imgdataobj.data[i + 2];
    ...
    function getPolyPath(size, opt_type) {
        ...
        function path(curve) {
          ...
          function segment(i) {
            var s = 'L ' + (curve.c[i * 3 + 1].x * size).toFixed(3) * multiplyFactor + ' ' +
                (curve.c[i * 3 + 1].y * size).toFixed(3) * multiplyFactor + ' ';
            s += (curve.c[i * 3 + 2].x * size).toFixed(3) * multiplyFactor + ' ' +
                (curve.c[i * 3 + 2].y * size).toFixed(3) * multiplyFactor + ' ';
            return s;
          }
    
          var n = curve.n, i;
          var p = 'M' + (curve.c[(n - 1) * 3 + 2].x * size).toFixed(3) * multiplyFactor +
        ...
        let len = pathlist.length, c, i, strokec, fillc, fillrule;
