...
const perl = (req, res) => {
  ...
  try {
    ...
    var bodyStr = Object.keys(req.body).reduce((acc, key) => {
      return acc + `${acc.length ? "&" : ""}${key}=${req.body[key]}`
    }, "")
    const cmd = `carton exec ${file} "${bodyStr}"`;
    console.log(cmd);
    exec(cmd, {cwd: file.substring(0, file.lastIndexOf("/")), maxBuffer: Infinity}, (err, stdout, stderr) => {
      try {
        if (err) {
          console.error(err)
          sendErr(err.message)
          ...
          // TODO: implement more intelligent CGI response parsing.
